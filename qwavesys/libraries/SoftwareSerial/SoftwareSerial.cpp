...
SoftwareSerial::~SoftwareSerial() {
   enableRx(false);
   if (m_rxValid)
      ObjList[m_rxPin & 0x1f] = NULL;
   if (m_buffer)
      free(m_buffer);
}

bool SoftwareSerial::isValidGPIOpin(int pin) {
   return ((pin & 0x1f) >= 0 && (pin & 0x1f) <= 16);
}

void SoftwareSerial::begin(long speed) {
   // Use getCycleCount() loop to get as exact timing as possible
   m_bitTime = ESP.getCpuFreqMHz()*1000000/speed;
   m_rxStartBitTime = m_bitTime + m_bitTime/3 -500;
   m_highSpeed = speed >= 9600;

   if(m_rxPin > 16 && speed > 19200) {
	  m_rxStartBitTime = 0;
   }
...
void SoftwareSerial::enableRx(bool on) {
   if (m_rxValid) {
      if (on)
         attachInterrupt(m_rxPin, ISRList[m_rxPin & 0x1f], m_invert ? RISING : FALLING);
      else
...
void ICACHE_RAM_ATTR SoftwareSerial::rxRead() {
   ...
   uint32_t wait = m_rxStartBitTime;
   unsigned long start = ESP.getCycleCount();
   uint8_t rec = 0;

   for (int i = 0; i < 8; i++) {
     //WAIT;
	 while (ESP.getCycleCount()-start < wait); wait += m_bitTime;
     rec >>= 1;
